...
fn update(state: State, event: event.Event) -> State {
  ...
  let tasks = case list.length(tasks) < 3 {
    True -> [l.center(folding.randomize(l.default_stack(0.25), 1)), ..tasks]
    False -> tasks
  }
  State(
    paper: folding.update(paper, event, fn(layer) {
      list.map(tasks, fn(task) {
        echo l.match(l.align(l.center(layer), 1.0), l.align(task, 1.0), 0.25)
      })
      layer
    }),
    tasks:,
